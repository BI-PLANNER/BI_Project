            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-300 flex flex-col md:flex-row items-stretch md:items-center gap-3">
              ...
              <div className="flex items-center gap-2 pt-2 md:pt-4">
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-gray-500 hover:text-gray-900 text-xs font-bold cursor-pointer"
                >
                  Cancelar
                </button>
              </div>
      ...
      {!hasAccess ? (
        <div className="glass-card p-8 rounded-3xl border border-slate-300 bg-white border border-slate-300 text-center space-y-4 shadow-sm">
          ...
          <p className="text-xs text-gray-600 max-w-lg mx-auto leading-relaxed">
            Este dashboard analítico de obligaciones y licitaciones contiene información directiva confidencial y está habilitado exclusivamente para el <strong className="text-amber-700">Gerente General</strong> ({assignedUserEmail}).
          </p>
          <div className="pt-2">
            <button
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 px-3 text-center font-mono font-bold text-xs">
                            <span className={item.fecha_cumplimiento.startsWith('2029') ? 'text-cyan-700' : 'text-gray-700'}>
                              {item.fecha_cumplimiento}
                            </span>
                          </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {reporteCumplimiento.map((resp, idx) => {
                ...
                return (
                  <div
                    key={resp.nombre}
                    className={`glass-card p-4 rounded-2xl border ${cardBorder} shadow-sm relative overflow-hidden transition-all hover:scale-[1.01]`}
                  >
                    {esTop && (
                      <div className="absolute top-3 right-3">
                        <Trophy className="w-5 h-5 text-amber-700 drop-shadow-sm" />
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <div
                        className="w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-black text-gray-900 shadow-sm shrink-0"
                    ...
                    {reporteCumplimiento.map((resp, idx) => {
                      ...
                      return (
                        <tr key={resp.nombre} className={`hover:bg-white/[0.04] transition-colors ${
                          ...
                          <td className="py-3 px-3 text-center">
                            <span className="font-bold text-emerald-700 font-mono">{resp.completados + resp.enPlazo}</span>
                          </td>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-600 max-w-3xl leading-relaxed">
                    Informe técnico de constancia de desarrollos, 21 tablas de base de datos, 4 APIs, scripts de Python OCR y 9 automatizaciones de <strong className="text-teal-700">Control Planner PRO</strong>.
                  </p>
                </div>
