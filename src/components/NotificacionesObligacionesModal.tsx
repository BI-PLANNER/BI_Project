...
export default function NotificacionesObligacionesModal({ isOpen, onClose }: NotificacionesModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 backdrop-blur-md p-4 overflow-y-auto">
      <div className="glass-card w-full max-w-4xl p-6 md:p-8 animate-scale-in border border-slate-300 shadow-sm rounded-3xl bg-white max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-slate-300">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-white border border-slate-300 text-amber-700 border border-slate-300">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="badge bg-indigo-500/20 text-indigo-700 font-mono text-[10px] border border-slate-300">
                  COMPRASAL / VERCEL
                </span>
              </div>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-xl text-gray-500 hover:text-gray-900 hover:bg-slate-50 transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Success Toast */}
        {sentSuccess && (
          <div className="mt-4 p-4 rounded-2xl bg-emerald-500/20 border border-slate-300 text-emerald-700 flex items-center gap-3 animate-fade-in text-xs font-semibold">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 my-4">
          <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-slate-300 flex items-center gap-3">
          ...
          <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-slate-300 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-emerald-700">jose.gomez • aaltunaher</p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-white/[0.03] border border-slate-300 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-black text-amber-700">26 hitos monitoreados</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
          {loading && !data ? (
            ...
            data?.resultados?.map((persona: any, idx: number) => (
              <div
                key={idx}
                className="p-3.5 rounded-2xl bg-white/[0.02] border border-slate-300 hover:border-slate-300 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-white border border-slate-300 text-indigo-700 font-bold text-xs flex items-center justify-center border border-slate-300">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="badge bg-slate-50 text-gray-600 text-[9px] px-1.5 font-mono">
                        {persona.totalTareas} {persona.totalTareas === 1 ? 'obligación' : 'obligaciones'}
                      </span>
                      {persona.criticas > 0 && (
                        <span className="badge bg-rose-500/20 text-rose-700 border border-slate-300 text-[9px] px-1.5 font-mono font-bold">
                ...
                <div className="flex items-center gap-2 self-end sm:self-auto">
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-gray-600 text-[11px] font-medium flex items-center gap-1.5 transition"
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-xl bg-emerald-500/10 hover:bg-gray-100 text-emerald-700 text-[11px] font-medium flex items-center gap-1.5 border border-slate-300 transition"
        ...
        {/* Footer Actions */}
        <div className="pt-4 mt-4 border-t border-slate-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={loading}
              className="btn-primary inline-flex items-center gap-2 px-5 py-2 text-xs font-bold shadow-sm disabled:opacity-50"
            >
      ...
      {previewOpen && selectedPersona && (
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-slate-50 p-4">
          <div className="glass-card w-full max-w-3xl p-6 rounded-3xl border border-slate-300 shadow-sm bg-white flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between pb-3 border-b border-slate-300">
              ...
              <button
                ...
                className="p-1.5 rounded-xl hover:bg-slate-50 text-gray-500 hover:text-gray-900"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto my-4 p-4 rounded-2xl bg-white border border-slate-300">
              <div
