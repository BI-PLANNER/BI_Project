...
async function run() {
  const detail = await getExecDetail(996);
  console.log('Exec 996 Status:', detail.data?.status);
  const runData = detail.data?.resultData?.runData || {};
  console.log('Executed nodes in 996:', Object.keys(runData));
  const syncNode = runData['Sincronizar Supabase: Licitaciones & Ofertas'];
  if (syncNode) {
    console.log('Sync node output:', JSON.stringify(syncNode[0]?.data?.main?.[0]?.[0]?.json, null, 2));
  }
}
